...
import { LogOut, Shield, Bell, HelpCircle } from 'lucide-react';
  ...
  useEffect(() => {
    if (user?.role === 'admin') {
      apiClient.get('/queries/admin')
        .then((res) => setPendingBadge(res.data.pendingCount || 0))
        .catch(() => {});
    }
  }, [user]);
    ...
    <>
      <header className="h-16 border-b border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl sticky top-0 z-40 px-3 sm:px-6 flex items-center justify-between transition-colors duration-300">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          {/* Helpdesk Query / Notification Bell */}
          <button 
            onClick={handleBellClick}
            title={user?.role === 'admin' ? 'View Admin Helpdesk Queries' : 'Submit Query / Profile Update'}
            className="p-2 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition relative"
          >
            <Bell className="w-4 h-4 sm:w-5 sm:h-5" />
            {user?.role === 'admin' && pendingBadge > 0 && (
              <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white font-extrabold text-[10px] rounded-full flex items-center justify-center animate-bounce shadow">
                {pendingBadge}
              </span>
            )}
          </button>
          ...
          <div className="flex items-center gap-2.5 sm:gap-3 pl-2 sm:pl-3 border-l border-slate-200 dark:border-slate-800">
            ...
            <div 
              ...
              title="Click to Submit Query / Profile Change Request"
              className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-sky-500/10 border border-sky-500/20 text-sky-500 flex items-center justify-center font-extrabold text-xs sm:text-sm shadow-sm shrink-0 cursor-pointer hover:scale-105 transition"
            >
      ...
      {/* Student / Teacher Query Helpdesk Modal */}
      <QueryModal
