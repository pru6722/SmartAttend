...
const QueryTicket_1 = __importDefault(require("../models/QueryTicket"));
...
class QueryController {
    ...
    static async createQuery(req, res) {
        try {
            ...
            const query = await QueryTicket_1.default.create({
                ticketId,
                senderId,
                senderName,
                senderRole,
    ...
    static async getMyQueries(req, res) {
        try {
            const senderId = req.user?.id;
            const queries = await QueryTicket_1.default.find({ senderId }).sort({ createdAt: -1 });
    ...
    static async getAllQueries(req, res) {
        try {
            const queries = await QueryTicket_1.default.find().sort({ createdAt: -1 });
            const pendingCount = await QueryTicket_1.default.countDocuments({ status: 'pending' });
    ...
    static async resolveQuery(req, res) {
        try {
            const { id } = req.params;
            const { adminResponse } = req.body;
            const query = await QueryTicket_1.default.findById(id) || await QueryTicket_1.default.findOne({ ticketId: id });
            if (!query) {
